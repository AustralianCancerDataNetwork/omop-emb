...
from omop_emb.backends.base_backend import EmbeddingBackendBase
...
def get_embedding_backend(
    ...
    *,
    ...
    if resolved == BackendType.PGVECTOR:
        try:
            from ..backends.pgvector import PGVectorEmbeddingBackend
    ...
    if resolved == BackendType.FAISS:
        try:
            from ..backends.faiss import FaissEmbeddingBackend
        except ImportError as exc:
            raise EmbeddingBackendDependencyError(
    ...
    if resolved == BackendType.FAISS_DB:
        try:
            from ..backends.faiss.faiss_db_backend import FaissEmbeddingBackendWithDBSupport
        except ImportError as exc:
            raise EmbeddingBackendDependencyError(
                "FAISS DB embedding backend requested but its dependencies are not "
                "available. Install the package with the FAISS extra using `pip install omop-emb[faiss]` or omop-emb[faiss-gpu]."
            ) from exc
        return FaissEmbeddingBackendWithDBSupport(base_dir=faiss_base_dir)
