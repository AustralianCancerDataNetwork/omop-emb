...
class EmbeddingStorageManager:
    ...
    @property
    def index_manager(self) -> Optional[BaseIndexManager]:
    ...
    def create_index_manager(
        ...
        if self._index_type == IndexType.FLAT:
            ...
            raise ValueError(f"Unsupported index type {self._index_type} for FAISS backend. Only {IndexType.FLAT} is supported currently.")
        
        self._index_manager = index_manager_cls(
        ...
        self._index_manager.populate_from_storage(
    ...
    def append(self, concept_ids: np.ndarray, embeddings: np.ndarray):
        ...
        if self.index_manager is not None:
            self.index_manager.add(concept_ids, embeddings)
    ...
    def search(
        ...
        if self.index_manager is None:
            self.create_index_manager(metric_type=metric_type)
        assert self.index_manager is not None, "Index manager should have been created by now."
